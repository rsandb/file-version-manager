...
class FileListTable extends \WP_List_Table {
	...
	public function column_file_name( $item ) {
		$actions = [ 
			...
			'delete' => sprintf( '<a href="%s" onclick="return confirm(\'Are you sure you want to delete this file?\')">Delete</a>', wp_nonce_url( admin_url( 'admin.php?page=file-version-manager&action=delete&file_id=' . $item['id'] ), 'delete_file_' . $item['id'] ) ),
		];
